...
func GetAllTokenQuotaTrendDates(c *gin.Context) {
	startTimestamp, err := strconv.ParseInt(c.Query("start_timestamp"), 10, 64)
	if err != nil {
		common.ApiErrorMsg(c, "invalid params")
		return
	...
	dates, err := model.GetTokenQuotaTrendData(0, startTimestamp, endTimestamp)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func GetTokenQuotaTrendDates(c *gin.Context) {
	userId := c.GetInt("id")
	startTimestamp, err := strconv.ParseInt(c.Query("start_timestamp"), 10, 64)
	if err != nil {
		common.ApiErrorMsg(c, "invalid params")
	...
	dates, err := model.GetTokenQuotaTrendData(userId, startTimestamp, endTimestamp)
	if err != nil {
		common.ApiError(c, err)
		return
	}
