...
import {
  formatPercent,
  usedVariantProgressClass,
  usedVariantTextClass,
...
/**
 * Radeon Cloud classifies failures with its own error codes, so it maps them
 * with a dedicated copy function under the uniform signature.
 */
...
export function RadeonCloudUsageDialog(
  ...
  const usedPercent = (() => {
    const fromPercent = toFiniteNumber(data?.daily_used_percent)
    if (fromPercent !== null) {
      return Math.max(0, Math.min(100, fromPercent * 100))
    }
    if (usedPoints !== null && allowancePoints !== null && allowancePoints > 0) {
      return Math.max(0, Math.min(100, (usedPoints / allowancePoints) * 100))
    }
    return null
  })()
  ...
  const lastRequestAt = (() => {
    const value = data?.last_24h_last_request_at
