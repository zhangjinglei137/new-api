...
test.each([
  ...
  async ({ type, label, url, savedUrl }) => {
    ...
    if (type === 45) {
      // Local VolcEngine UI: the clickable unlock label is "Access Mode"
      // (the locked view shows the standard/coding/custom mode select).
      // After 10 clicks the custom "API Base URL" input with the server
      // placeholder is revealed.
      const accessModeLabel = screen.getByText('Access Mode')
      for (let click = 0; click < 10; click++) {
        fireEvent.click(accessModeLabel)
      }
    }
