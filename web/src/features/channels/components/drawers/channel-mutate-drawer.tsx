...
export function ChannelMutateDrawer({
  ...
  const currentEndpointProfile = formValues.endpoint_profile
  const {
  ...
  const previewRequest = useMemo<ChannelModelDiscoveryRequest>(
    () => ({
      ...
      data: {
        ...
        proxy: currentProxy,
        endpoint_profile: currentEndpointProfile,
      },
    }),
    [
      currentType,
  ...
  const modelProxyRulesFieldsUi = (
    <FormField
      control={form.control}
      name='model_proxy_rules'
      render={() => (
        <FormItem>
          <FormLabel>{t('Model Proxy Rules')}</FormLabel>
          <FormDescription>
            {t(
          ...
          <div className='space-y-2'>
            {modelProxyRulesFields.fields.map((ruleField, index) => (
              <div key={ruleField.id} className='rounded-lg border p-3'>
                <div className='flex items-start gap-2'>
                  <div className='grid flex-1 gap-2'>
                    <Input
                      placeholder={t('gpt-5.6-luna, regex:^gpt-')}
                      aria-label={t('Models')}
                      {...form.register(`model_proxy_rules.${index}.models`)}
                    />
                    <Input
                      placeholder={t('socks5://user:pass@host:port')}
                      aria-label={t('Proxy Address')}
                      {...form.register(`model_proxy_rules.${index}.proxy`)}
                    />
                  </div>
                  <Button
                    type='button'
                    ...
                    onClick={() => modelProxyRulesFields.remove(index)}
                  >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                {t('Used to order channels in the list')}
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
      ...
      <ChannelApiAccessSection>
        ...
        <div className='border-border/60 bg-muted/10 rounded-lg border p-4'>
          ...
          >
            ...
            {/* VolcEngine (type 45) */}
            {/* VolcEngine (type 45) - Access mode (standard / coding plan / custom) */}
            {currentType === 45 && !doubaoApiEditUnlocked && (
              <div className='flex flex-col gap-4'>
                <FormField
                  ...
                  render={() => (
                    <FormItem>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => {
                          ...
                          if (mode === 'coding') {
                            ...
                            form.setValue('endpoint_profile', '', {
                              shouldDirty: true,
                              shouldValidate: true,
                            })
                          }
                      ...
                      <FormDescription>
                        {t('Select the access mode for this VolcEngine channel')}
                      </FormDescription>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                {doubaoAccessMode === 'coding' && (
                  <>
                    <FormItem>
                      <FormLabel>{t('Coding Plan Endpoints')}</FormLabel>
                      <FormControl>
                        <Input
                          readOnly
                    ...
                    <Select
                      ...
                      items={[
                        ...
                        ...(currentType === 26
                          ? [
                              {
                                value: 'coding-intl',
                                label: t('Coding Plan (International)'),
                              },
                            ]
                          : []),
                      ]}
                      onValueChange={(value) => {
                        ...
                        if (mode === 'standard') {
                          field.onChange('')
                          if (currentType === 26) {
                            form.setValue('base_url', 'https://open.bigmodel.cn', {
                              shouldDirty: true,
                            })
                          } else {
                            form.setValue('base_url', 'https://api.moonshot.cn', {
                              shouldDirty: true,
                            })
                          }
                        } else {
            ...
            {currentType === CHANNEL_TYPE_COMMANDCODE && (
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('Usage query cookie')}</FormLabel>
                    <FormControl>
                      <Input
                        type='password'
