...
import { createModel, updateModel, getModel, getVendors, getEndpointDefinitions } from '../../api'
import { CapabilityGroupsEditor } from '../capability-groups-editor'
import { getNameRuleOptions, ENDPOINT_TEMPLATES } from '../../constants'
import { endpointDefinitionsQueryKeys, modelsQueryKeys, vendorsQueryKeys } from '../../lib'
import {
  modelFormSchema,
...
export function ModelMutateDrawer(props: {
  ...
  const handleFillEndpointTemplate = (key: string) => {
    const apiDefinitions = endpointDefinitionsData?.data?.endpoints
    const definition = apiDefinitions?.find((entry) => entry.type === key)
    if (definition) {
      const { display_name: _displayName, npm: _npm, ...template } = definition
      form.setValue(
        'endpoints',
        JSON.stringify({ [key]: template }, null, 2),
        { shouldDirty: true }
      )
      return
    }
                  ...
                  >
                    ...
                    <SideDrawerSection>
                      <div className='flex items-center justify-between'>
                        ...
                        <Combobox
                          options={endpointTemplateOptions.map((option) => ({
                            value: option.key,
                            label: option.label,
                          }))}
